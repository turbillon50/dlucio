    ...
    <div className="px-4 pt-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        {kpis.map((k, i) => (
          ...
          >
            ...
            <p className="mt-0.5 text-[13px] font-semibold text-white/80">{k.label}</p>
      ...
      <Link href="/admin/resultados">
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[13px] text-white/55">{ADMIN_KPIS.partidosPorCerrar} partidos de la Jornada 14 por cerrar</p>
          </div>
          <Ico.arrowUp size={18} className="rotate-90 text-[#d4ba28]" />
        </motion.div>
      </Link>
      ...
      <div className="card mt-4 p-4">
        ...
        <div className="mt-3 flex h-7 overflow-hidden rounded-lg">
          ...
          <div className="flex items-center justify-center bg-[#010173] text-[11px] font-bold text-white" style={{ width: "30%" }}>
            30% Casa
          </div>
        </div>
        <div className="mt-3 flex justify-between text-[13px]">
          <div>
            <p className="text-white/45">Al ganador</p>
            <p className="font-black text-[#d4ba28]">${Math.round(POOL.total * 0.7).toLocaleString("es-MX")}</p>
          </div>
          <div className="text-right">
            <p className="text-white/45">Para la casa</p>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Link href="/admin/jornadas" className="text-xs font-semibold text-[#d4ba28]">Gestionar →</Link>
      </div>
      <div className="mt-3 space-y-2">
        {CURRENT_ROUND.slice(0, 4).map((m) => (
          <div key={m.id} className="card flex items-center justify-between p-3 text-sm">
            <span className="font-medium">{teamById(m.home).name} <span className="text-white/30">vs</span> {teamById(m.away).name}</span>
